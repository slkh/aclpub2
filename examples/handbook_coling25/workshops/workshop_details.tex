...
% SAQIB: No time posted in workshop page. I made an educated guess. 
...
\begin{itemize}
    \item Analyzing faith-defining canons and authoritative interpretations
    \item Extracting insights from sermons, liturgy, prayers, and poetry
    \item Leveraging Large Language Models for novel research avenues
\end{itemize}
...
\noindent{VarDial is a well-established series of workshops promoting a forum for scholars working on a range of topics related to the study of diatopic language variation from a computational perspective.
...
While many workshops have been devoted to the building of educational software (https://sig-edu.org/bea/2024), the development of intelligent writing assistants (https://in2writing.glitch.me), or the evaluation of written text, they were generally only concerned with the formal aspects, that is, the lower end of writing: grammatical error detection, spotting of spelling mistakes, etc.

Yet writing or text composition require much more: 
\begin{itemize}
    \item texts must be readable, and to achieve this goal authors must be able to  do  more than just produce a set of correct forms. Sentences must be organized, point into some direction, and the whole (resulting text) must convey a point.
    \item the hardest and most important part of composition is actually not the very act of writing (conversion of ideas into a set of grammatically correct and well spelled linguistic forms), but the carrying out of the preceding steps, which normally are planning, reasoning, and thinking. 
\end{itemize}
...
\begin{itemize}
    \item to find ideas, 
    \item to evaluate them (truth, relevancy) and 
    \item to organize them in such a way that the whole makes sense (coherence) for the reader, allowing her  to get the author's point or understand his line of reasoning. 
\end{itemize}

While all this sounds obvious, this seems to be the first workshop

\begin{itemize}
    \item to consider the entire spectrum of writing (planning: ideation, organization; drafting/expression; editing; revision); 
    \item to include the higher levels of composition, 
    \item to integrate right from the start humans into the loop of the development cycle, and 
    \item to consider support and feedback at any moment (before, during, and after writing) and not only at the very end.
\end{itemize}
